...
def main_ranking(_):
	...
	instance_scope = np.load(export_path + 'train_instance_scope.npy')
        instance_scope_path = np.load(export_path_g + 'train_instance_scope_path.npy')
	...
	train_pos2 = np.load(export_path + 'train_pos2.npy')
        ...
        train_word_cross = np.load(export_path_g + 'train_word_cross.npy')
        train_pos1_cross = np.load(export_path_g + 'train_pos1_cross.npy')
        train_pos2_cross = np.load(export_path_g + 'train_pos2_cross.npy')

        train_comp_fea = complexity_features(train_word_cross)
	...
	tf.summary.scalar('learning_rate_kg', FLAGS.learning_rate_kg)
        ...
        print 'building evidences encoder ...'
	...
	saver = tf.train.Saver(max_to_keep=None)
        ...
        saver.restore(sess, FLAGS.model_dir+'pretrain_'+str(FLAGS.max_epoch_pre))#the pretrained model via Textual/Hybrid/KG paths
	...
	def train_nn(coord):
		...
		for one_epoch in range(FLAGS.max_epoch):
			...
			print('ranking epoch '+str(one_epoch+1)+' starts!')
			...
			for i in range(int(len(train_order)/float(FLAGS.batch_size))):
				...
				output, loss, correct_predictions = train_step(train_head[index], train_tail[index], train_word[index,:], train_pos1[index,:], train_pos2[index,:],
                                                                               ...
                                                                               np.array(scope_path), train_head_path, train_tail_path, train_comp_fea[index_path,:])
				num = 0
				s = 0
				losstot += loss
				...
				time_str = datetime.datetime.now().isoformat()
				print "ranking epoch %d step %d time %s | loss : %f, not NA accuracy: %f" % (one_epoch, i, time_str, loss, s2 / tot2)
				current_step = tf.train.global_step(sess, global_step)
                                
                        if (one_epoch + 1) % save_epoch == 0 and (one_epoch + 1) >= FLAGS.max_epoch:
				print 'epoch '+str(one_epoch+1)+' has finished'
				print 'saving model...'
				path = saver.save(sess,FLAGS.model_dir+FLAGS.model+'_pretrain_ranking_' + str(FLAGS.max_epoch))
