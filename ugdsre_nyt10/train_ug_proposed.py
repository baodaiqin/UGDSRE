...
def main(_):
	...
	def train_nn(coord):
		...
		for one_epoch in range(FLAGS.max_epoch):
			...
			#if (one_epoch + 1) % save_epoch == 0 and (((one_epoch + 1) >= 98 and (one_epoch + 1) <= 100) or ((one_epoch + 1) >= 148 and (one_epoch + 1) <= 150)):
                        if (one_epoch + 1) % save_epoch == 0 and (one_epoch + 1) >= 2:
