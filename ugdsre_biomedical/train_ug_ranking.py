...
export_path_g = "/home/dq/biomedical_part1/"
...
tf.app.flags.DEFINE_integer('katt_flag', 13, '0 for 1hop, 1 for 1+2hop, 2 for 1+2+3hop, 3 for 2hop only, 4 for 3hop only, 5 for 1+3hop')
...
def main(_):
	...
	saver = tf.train.Saver(max_to_keep=None)
        ...
        #KG.collect_all_batch(batch_size)
	...
	def train_nn(coord):
		...
		for one_epoch in range(FLAGS.max_epoch):
			...
			for i in range(int(len(train_order)/float(FLAGS.batch_size))):
				...
				time_str = datetime.datetime.now().isoformat()
				print "batch %d step %d time %s | loss : %f, not NA accuracy: %f" % (one_epoch, i, time_str, loss, s2 / tot2)
				current_step = tf.train.global_step(sess, global_step)
                                
			#if (one_epoch + 1) % save_epoch == 0 and (((one_epoch + 1) >= 44 and (one_epoch + 1) <= 50) or ((one_epoch + 1) >= 94 and (one_epoch + 1) <= 100)):
                        if (one_epoch + 1) % save_epoch == 0 and (one_epoch + 1) >= 48:
