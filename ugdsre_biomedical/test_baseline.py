...
export_path_g = "/home/dq/biomedical_part2/"
...
tf.app.flags.DEFINE_string('checkpoint_path','./model_saved/','path to store model')
...
def main(_):
	...
	test_instance_scope = np.load(export_path + 'test_instance_scope%s.npy'%test_id)
        test_instance_scope_path = np.load(export_path_g + 'test_instance_scope_kg%s.npy'%test_id)
        test_instance_scope_path3 = np.load(export_path_g + 'test_instance_scope_tx%s.npy'%test_id)
        test_instance_scope_path4 = np.load(export_path_g + 'test_instance_scope_ug%s.npy'%test_id)
	...
	test_pos2 = np.load(export_path + 'test_pos2%s.npy'%test_id)
        ...
        test_word_cross = np.load(export_path_g + 'test_word_kg%s.npy'%test_id)
        test_pos1_cross = np.load(export_path_g + 'test_pos1_kg%s.npy'%test_id)
        test_pos2_cross = np.load(export_path_g + 'test_pos2_kg%s.npy'%test_id)

        test_word_cross3 = np.load(export_path_g + 'test_word_tx%s.npy'%test_id)
        test_pos1_cross3 = np.load(export_path_g + 'test_pos1_tx%s.npy'%test_id)
        test_pos2_cross3 = np.load(export_path_g + 'test_pos2_tx%s.npy'%test_id)

        test_word_cross4 = np.load(export_path_g + 'test_word_ug%s.npy'%test_id)
        test_pos1_cross4 = np.load(export_path_g + 'test_pos1_ug%s.npy'%test_id)
        test_pos2_cross4 = np.load(export_path_g + 'test_pos2_ug%s.npy'%test_id)
	...
	f.write('iteration\taverage precision\n')
        ...
        #saver.restore(sess, './model_saved/cnn%s-402000'%FLAGS.katt_flag)
        saver.restore(sess, './pretrained_baseline_bio/cnn%s-402000'%FLAGS.katt_flag)
	...
	average_precision = average_precision_score(exclude_na_flatten_label,exclude_na_flatten_output, average = "micro")
        if katt_flag == 10:
                np.save('./result/'+FLAGS.model+'+_sent_label'+'.npy',exclude_na_flatten_label)
                np.save('./result/'+FLAGS.model+'+_sent_prob'+'.npy', exclude_na_flatten_output)
        elif katt_flag == 11:
                np.save('./result/'+FLAGS.model+'+_sent_kg_label'+'.npy',exclude_na_flatten_label)
                np.save('./result/'+FLAGS.model+'+_sent_kg_prob'+'.npy', exclude_na_flatten_output)
